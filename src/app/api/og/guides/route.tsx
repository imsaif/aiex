...
import { OG, GUIDE_BRAND_ICONS, generateGridPaths, loadFonts } from '../shared';
...
export async function GET(request: NextRequest) {
  ...
  if (!slug) {
    return new Response('Missing guide slug', { status: 400 });
  }
  ...
  const brandIcon = GUIDE_BRAND_ICONS[guide.tool] || GUIDE_BRAND_ICONS['GitHub'];
  const iconPaths = brandIcon.paths;
  const iconType = brandIcon.type;

  const lessonCount = guide.lessons?.length || 0;
  const stats = [
    lessonCount > 0 ? `${lessonCount} Lessons` : null,
    guide.readTime ? `${guide.readTime} min` : null,
    guide.skillLevel || null,
  ]
    .filter(Boolean)
    .join('  ·  ');
    ...
    (
      ...
      >
        ...
        >
          ...
          >
            <svg
              ...
              height="80"
              viewBox="0 0 24 24"
              fill={iconType === 'fill' ? OG.white : 'none'}
              stroke={iconType === 'stroke' ? OG.white : 'none'}
              strokeWidth={iconType === 'stroke' ? '1.2' : '0'}
              strokeLinecap="round"
        ...
        >
          ...
          >
            {guide.tool.toUpperCase()} COURSE
          </div>

          {/* Guide title */}
          <div
            style={{
              fontSize: guide.title.length > 28 ? '64px' : '76px',
              fontWeight: 700,
              color: OG.white,
              lineHeight: 1.05,
