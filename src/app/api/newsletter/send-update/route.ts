...
async function sendPatternUpdateEmail(
  ...
  const patternsHtml = patterns
    .map(
      (pattern) => `
        <div style="background: #f9fafb; border-radius: 8px; padding: 20px; margin-bottom: 20px; border-left: 4px solid #667eea;">
          <h3 style="margin: 0 0 10px 0; color: #667eea; font-size: 20px;">
            ${pattern.title}
          </h3>
          <p style="color: #6b7280; margin: 0 0 15px 0; font-size: 14px; text-transform: uppercase; font-weight: 600;">
            ${pattern.category}
          </p>
