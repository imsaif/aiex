...
async function getPublishedDrafts(): Promise<Newsletter[]> {
  try {
    ...
    // (~6KB JSON per row) and `sources`/`createdAt`/`updatedAt` aren't
    ...
    //
    // `content` is deliberately NOT selected: the list only ever needed it to
    // count words for "N min read" and to spot empty quiet-day entries, but it
    // is the single heaviest column here (~18KB/row, ~1MB across these 60 rows
    // — three times the structuredData it was avoiding). Both facts are now
    // precomputed into `readMinutes` at write time. Re-adding `content` here
    // would undo the largest saving on this query.
    ...
    return drafts.map((draft) => ({
      ...
      // Rows predating the backfill have null; treat as 0 so they render inline
      // rather than claiming a bogus reading time.
      readMinutes: draft.readMinutes ?? 0,
    }));
  } catch (error) {
...
export default async function NewsPage() {
  ...
  // The two static fallback newsletters live in a TS file, not the DB, so they
  // have no precomputed columns — tag them here from the fields they do have.
  // DB rows already carry `products`/`readMinutes` and must not be recomputed:
  // theirs were derived from structuredData, which isn't available on this page.
  const staticTagged = staticNewsletters.map((n) => ({
    ...n,
    products: productsForIssue(n.title, n.summary, undefined),
    readMinutes: computeReadMinutes(n.content),
  }));

  // Combine and sort by date (newest first)
  const allNewsletters = [...staticTagged, ...uniqueDbNewsletters]
    .sort((a, b) => {
