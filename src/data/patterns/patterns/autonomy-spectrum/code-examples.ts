...
export const codeExamples: CodeExample[] = [
  {
    title: "AI Agent Autonomy Settings Panel",
    description: "An interactive settings panel where users configure per-task autonomy levels for an AI email agent, with a live preview of what the agent will do at each level.",
    language: "tsx",
                ...
                {AUTONOMY_LEVELS.map((level) => (
                  <button
                    ...
                    className={\`flex-1 h-2 rounded-full transition-all duration-200 cursor-pointer \${
                      level.id <= task.level ? AUTONOMY_LEVELS[task.level].color : 'bg-gray-200 dark:bg-gray-700'
