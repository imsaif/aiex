...
export function ThemeToggle() {
  ...
  return (
    ...
    >
      {/* Sun icon - shown in light mode, morphs to moon on hover */}
      <div
        className={`transition-all duration-500 ${
          isDark ? 'opacity-0 -rotate-180 scale-0' :
          isHovering ? 'opacity-0 rotate-180 scale-0' :
      ...
      {/* Moon icon - shown in dark mode or on hover */}
      <div
        className={`absolute top-1.5 left-1.5 transition-all duration-500 ${
          isDark ? 'opacity-100 rotate-0 scale-100' :
          isHovering ? 'opacity-100 rotate-0 scale-100' :
